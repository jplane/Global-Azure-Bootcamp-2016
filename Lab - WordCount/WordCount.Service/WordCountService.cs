        ...
        protected override async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        var dequeueReply = await inputQueue.TryDequeueAsync(tx);

                        if (dequeueReply.HasValue)
                        {
                            var word = dequeueReply.Value;

                            var count = await wordCountDictionary.AddOrUpdateAsync(tx, word, 1, (key, oldValue) => oldValue + 1);

                            var numberOfProcessedWords = await statsDictionary.AddOrUpdateAsync(
                                tx, "Number of Words Processed", 1, (key, oldValue) => oldValue + 1);

                            var queueLength = await inputQueue.GetCountAsync(tx);

                            await tx.CommitAsync();

                            ServiceEventSource.Current.RunAsyncStatus(this.Partition.PartitionInfo.Id, numberOfProcessedWords, queueLength, word, count);
                        }
                    }
